    ...
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-2xs">
      <div className="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          {/* Quick toggle mode (Guest / User) */}
          <button
            onClick={() => setIsLoggedIn(!isLoggedIn)}
            className="hidden sm:block text-[11px] font-medium text-slate-500 hover:text-indigo-600 bg-slate-100 hover:bg-slate-200 px-2.5 py-1 rounded-lg transition"
            title="Toggle between Guest View and Dashboard"
          >
            {isLoggedIn ? (lang === 'km' ? 'មើល Landing' : 'Guest Mode') : (lang === 'km' ? 'ចូលរៀន' : 'Dashboard')}
          </button>
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-indigo-100 border border-indigo-200 flex items-center justify-center text-indigo-700 font-bold text-xs hover:ring-2 hover:ring-indigo-500/20 transition cursor-pointer"
