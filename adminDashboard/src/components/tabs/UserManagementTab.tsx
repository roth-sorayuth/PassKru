      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="ស្វែងរកអ្នកប្រើប្រាស់ តាមឈ្មោះ អ៊ីមែល ឬលេខទូរស័ព្ទ..."
              value={userSearch}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100">
          <span className="text-xs font-normal text-slate-500 uppercase tracking-wider mr-1">តួនាទី:</span>
          <button
                  ...
                  <tr key={u.userId} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className="py-3.5 px-4">
                      <span className="inline-flex items-center gap-1 text-xs font-normal text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">
                        <Flame className="w-3 h-3 text-slate-500" /> {u.streakDays || 0} ថ្ងៃ
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-xs text-slate-600 font-normal">
                      {new Date(u.createdAt).toLocaleDateString('km-KH', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </td>
